...
try {
  ...
  for(weekday of days){
    ...
    if(emp.availableNWdays.length<emp.nwdaycount){
      emp.nwdaycount=emp.availableNWdays.length}
   
  }
} catch (err) {
  console.error(
    "Error at initializing office days of employees... Please make sure the sum of fixed day count of employees matches with the employee count that defined in 'days' array"
  );
  console.error(err);
...
try {
  for(emp of employees) {
    ...
    for (let l = 0; l < emp.offdays.phdays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===emp.offdays.phdays[l].toLowerCase()){
