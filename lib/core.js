...
try {
  ...
  emp.availableNWdays = JSON.parse(JSON.stringify(days));
  
    for (let nwday in emp.nwdays) {
      
      let findViableday =emp.availableNWdays.findIndex((object) => {
    ...
    for (let ofday in emp.offdays.aldays) {
      let findoffdayIndex =emp.availableNWdays.findIndex((object) => {
...
try {
  ...
  console.log("----------")
  console.log(JSON.stringify(emp.name) +""+JSON.stringify(emp.availableNWdays))
    for(let j = emp.nwdaycount - 1; j >= 0; j--) {
      //employee's nwday count times iteration
     
      let index = 0;
      if (!isBalanced) {
        for (let k = 0; k <emp.availableNWdays.length; k++) {
          if (
          emp.availableNWdays[k].employeeCount >emp.availableNWdays[index].employeeCount
          ) {
            index = k;
            isBalanced = true;
          }
      ...
      if (!emp.nwdays.includes(emp.availableNWdays[index].day)) {
        emp.nwdays.push(emp.availableNWdays[index].day);
      }
      findIndex = days.findIndex((object) => {
        return object.day ===emp.availableNWdays[index].day;
      });
      emp.nwdaycount--;
      days[findIndex].employeeCount--;
    emp.availableNWdays.splice(index, 1);
