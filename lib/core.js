...
try {
  for (let i = 0; i < employees.length; i++) {
    ...
    for (let ofday in employees[i].offdays.aldays) {
      let findoffdayIndex = availabledays.findIndex((object) => {
        return object.day === employees[i].offdays.aldays[ofday];
      });
      availabledays.splice(findoffdayIndex, 1);
    }
    for (let ofday in employees[i].offdays.hrdays) {
      let findoffdayIndex = availabledays.findIndex((object) => {
        return object.day === employees[i].offdays.hrdays[ofday];
      });
      availabledays.splice(findoffdayIndex, 1);
    }
    for (let ofday in employees[i].offdays.mldays) {
      let findoffdayIndex = availabledays.findIndex((object) => {
        return object.day === employees[i].offdays.mldays[ofday];
      });
      availabledays.splice(findoffdayIndex, 1);
    }
    for (let ofday in employees[i].offdays.swdays) {
      let findoffdayIndex = availabledays.findIndex((object) => {
        return object.day === employees[i].offdays.swdays[ofday];
      });
      availabledays.splice(findoffdayIndex, 1);
    }
    for (let ofday in employees[i].offdays.uldays) {
      let findoffdayIndex = availabledays.findIndex((object) => {
        return object.day === employees[i].offdays.uldays[ofday];
      });
      availabledays.splice(findoffdayIndex, 1);
    }
  }
...
try {
  for (let i = 0; i < employees.length; i++) {
    ...
    for (let l = 0; l < employees[i].offdays.aldays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===employees[i].offdays.aldays[l].toLowerCase()){
          employeeObj[`${Object.keys(employeeObj)[k]}`] = "AL";
        }
      }
    }
    for (let l = 0; l < employees[i].offdays.hrdays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===employees[i].offdays.hrdays[l].toLowerCase()){
          employeeObj[`${Object.keys(employeeObj)[k]}`] = "HR";
        }
      }
    }
    for (let l = 0; l < employees[i].offdays.mldays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===employees[i].offdays.mldays[l].toLowerCase()){
          employeeObj[`${Object.keys(employeeObj)[k]}`] = "ML";
        }
      }
    }
    for (let l = 0; l < employees[i].offdays.swdays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===employees[i].offdays.swdays[l].toLowerCase()){
          employeeObj[`${Object.keys(employeeObj)[k]}`] = "SW";
        }
      }
    }
    for (let l = 0; l < employees[i].offdays.uldays.length; l++) {
      for (let k = 0; k < Object.keys(employeeObj).length; k++) {
        if (Object.keys(employeeObj)[k].toLowerCase() ===employees[i].offdays.uldays[l].toLowerCase()){
